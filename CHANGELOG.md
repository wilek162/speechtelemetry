...
### Added
- **Schema v1.0 — `SpeakerProfile` and per-speaker analytics**: New `SpeakerProfile` dataclass in `types.py` records per-speaker aggregate statistics: `speaking_time_s`, `turn_count`, `word_count`, `mean_segment_confidence`, `dominant_emotion`, and `emotion_distribution` (mean probability per label across all emotional segments). `TranscriptDocument` gains `speakers: list[SpeakerProfile] | None` (populated when diarization runs) and `schema_version = "1.0"`. Both exported from `speechtelemetry` public API. Speaker profiles are computed by `_compute_speaker_profiles()` in `core/pipeline.py` after all pipeline stages complete.
- **JSON output: clean, ordered, cross-platform**: `json_exporter.py` now (1) rounds all float values to 4 decimal places eliminating `0.6819999999999999` and `2.14e-12` noise, (2) normalizes `source_path` to forward slashes for cross-platform portability, (3) controls field ordering: `schema_version` first, `speakers` summary before `segments` for quick document scanning, (4) omits `speakers` when diarization was disabled.
- **SRT and VTT: timestamp overlap fix**: ASR segment boundaries commonly have 10–50ms overlaps (seg[i].end > seg[i+1].start). Both exporters now clamp each cue's start to `max(start, prev_end)` so no two subtitle cues can display simultaneously. VTT gains a `NOTE speechtelemetry` metadata block identifying the source file.
- **TextGrid: valid Praat intervals with emotion and word tiers**: Rebuilt from scratch. Previous version emitted overlapping intervals (invalid in Praat). New `_normalize()` helper builds contiguous non-overlapping intervals covering `[0, xmax]` with empty-text gaps. Produces 4 tiers: `utterances` (text + speaker prefix), `speakers`, `emotion` (dominant label per segment), `words` (individual aligned words — only when word timestamps present).
